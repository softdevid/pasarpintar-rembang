...
function Show({ dataShow, props, handleCloseShow }) {
  ...
  const [diskon, setDiskon] = useState(dataShow.diskon);
  const [tglAwalDiskon, setTglAwalDiskon] = useState(dataShow.tglAwalDiskon);
  const [tglAkhirDiskon, setTglAkhirDiskon] = useState(dataShow.tglAkhirDiskon);

  const handleUpdate = () => {
    const data = { diskon, tglAwalDiskon, tglAkhirDiskon, id: dataShow.id }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-4">
        <h1 className="font-bold text-2xl">{dataShow.namaProduk}</h1>
        <div className="text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-3">
        <div className="justify-center mx-auto">
          {dataShow.imgUrl ? (
            ...
            <img src={dataShow.imgUtama} className="w-80 h-80" />
          )}
        </div>
        <div className="grid grid-cols-1">
          <div className="text-lg ">Nama Produk: {dataShow.namaProduk}</div>
          <div className="text-lg ">Terjual: {dataShow.terjual}</div>
          <div className="text-lg ">{dataShow.jenisHarga}: {dataShow.terjual}</div>
          <div className="text-lg ">Stok toko: {dataShow.stokToko}</div>
          <div className="text-lg ">Stok gudang: {dataShow.stokGudang}</div>
          <div>Diskon: {rupiah(dataShow.diskon)}</div>
          <div>Awal diskon: {dataShow.tglAwalDiskon}</div>
          <div>Akhir diskon: {dataShow.tglAkhirDiskon}</div>
